...
async function populateFields() {
    account.getUser().then(function(user) {
        ...
        buttons.forEach(function(button) {
            var sku = button.name;
            button.setAttribute('data-pid', button.getAttribute([userIsPremium ? 'data-premium-id' : 'data-pid']));
        });
...
async function refreshCart(target) {
    ...
    if (target != null) {
        ...
        var productId = document.getElementsByName(targetName)[0].getAttribute(userIsPremium ? 'data-premium-id' : 'data-pid');
        query += '?add-to-cart=' + productId + '&quantity=' + quantity + '&ts=' + slug;
        var updated_cart = await cart.updateCartItem(userId, parseInt(productId), quantity);
        console.log('カート入れました', updated_cart);
    }
    var opts = { method: 'GET' };
    // console.log('カートの状態', await cart.getCartDetails());

    // fetch(query, opts).then(function(res) { return res.json() }).then(async function(json) {
        // console.log('json', json);
        console.log('cookie', cart.getCartContents());
        var cart_json = await cart.getCartDetails();
        var details = cart_json.cart_details;
        console.log('cart_json', cart_json);
        var items = '';
        var tax_exclusive = 0;
        for (var i=0; i < details.length; i++) {
            if (details[i].buylater == 1) continue;
            var pid = details[i].id;
            var cart_id = details[i].cart_id;
            var product_qty = details[i].quantity;
            ...
            var product_name = details[i].name;
            var product_price = details[i].price;
...
onPageLoaded(async function() {
    ...
    // タイムセールスラグ
    var slug = location.pathname.replace(/\/+$/, '').substring(location.pathname.replace(/\/+$/, '').lastIndexOf('/') + 1);
    cookieService.setCookie('_v_timesale_slug', slug);
    ...
    if (document.getElementById('login-btn') != null) {
        document.getElementById('login-btn').addEventListener('click', function (event) {
            ...
            document.getElementById('exception-error-message').style.display = 'none';
            document.getElementById('registration-error-message').style.display = 'none';
            account.authenticateUser(username, password).then(async function () {
                await populateFields();
